...
def calculate(
    *,
    ...
    for row in data_np:
        mahalanobis_distance = distance.mahalanobis(row, mean, np.linalg.inv(covariance_matrix))
        mahalanobis_distances.append(mahalanobis_distance)

    # Now `mahalanobis_distances` contains the Mahalanobis distances for all rows
    aligned_pl = aligned_pl.with_columns(pl.Series("mahalanobis", mahalanobis_distances))

    std = aligned_pl.std()
    avg = aligned_pl.mean()

    aligned_pl = aligned_pl.with_columns(pl.when(pl.col('mahalanobis')>(avg['mahalanobis'] + (3*std['mahalanobis']))).then(pl.lit('Yes')).otherwise(pl.lit('No')).alias('outlier'))
    return(aligned_pl)
